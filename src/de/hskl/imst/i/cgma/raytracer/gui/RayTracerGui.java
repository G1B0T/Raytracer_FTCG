...
import de.hskl.imst.i.cgma.raytracer.STLImporter;
    ...
    if (ret == JFileChooser.APPROVE_OPTION) {
        ...
        try {
            ...
            if (fileName.endsWith(".stl")) {
                // Wenn STL-Datei: mit STLImporter laden
                T_Mesh mesh = STLImporter.loadSTL(selectedFile, 1.0f); // Scale nach Wunsch anpassen
                addObject(mesh);           // Objekt zur Szene hinzufügen
                callSgChangedListeners();  // Szene updaten, falls nötig
                prevFolderPath = selectedFile.getParent();
                System.out.println("STL-Datei erfolgreich hinzugefügt.");
            } else {
                // Für andere Dateitypen wie bisher
                Class<? extends RTFile> type = RTFile.getType(selectedFile);
                if (type != null) {
                    if (RT_Object.class.isAssignableFrom(type)) {
                        RT_Object obj = (RT_Object) RTFileReader.read(type, selectedFile);
                        addObject(obj);
                        callSgChangedListeners();
                        prevFolderPath = selectedFile.getParent();
                        System.out.println("Objekt erfolgreich hinzugefügt.");
                    } else {
                        throw new IOException("Dateityp nicht unterstützt.");
                    }
                } else {
